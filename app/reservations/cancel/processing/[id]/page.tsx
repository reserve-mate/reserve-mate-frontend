...
export default function MatchCancelProcessingPage({ params }: { params: { id: string } }) {
  ...
  const cancelId = searchParams.get("reservationNumber")
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        const result = await paymentService.cancelReservationChk(cancelId);
    ...
    <div className="container max-w-md mx-auto py-8 px-4">
      <Card className="border-0 shadow-lg rounded-xl overflow-hidden">
        <CardHeader className="text-center">
          <CardTitle className="text-2xl">예약 취소 처리</CardTitle>
          <CardDescription>
